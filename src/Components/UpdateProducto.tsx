...
const UpdateProducto = () => {
  ...
  const save = async () => {
    const response = await fetch(`${appsetings.apiUrl}Productos/Update`, {
      method: "PUT",
      headers: {
  ...
  useEffect(() => {
    const getProducto = async () => {
      const response = await fetch(`${appsetings.apiUrl}Productos/Get/${id}`);
      if (response.ok) {
  ...
  return (
    <div className="container mt-5">
      <h1 className="text-center mb-5">Actualizar Producto</h1>
        ...
        <div className="w-50">
          <form className="formUpdate">
            ...
            <div className="mb-3">
              <button
                type="button"
                className="btn btn-primary w-100 w-sm-25 mb-2"
                onClick={save}
              >
                Guardar
              </button>
